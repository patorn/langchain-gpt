...
def main():
    ...
    # Deprecated ConversationBufferMemory replaced by RunnableWithMessageHistory
    ...
    def format_history(session_id: str = "default") -> str:
        ...
        lines = []
        for i, m in enumerate(history.messages, 1):
    ...
    memory_tool = Tool(
        name="ShowMemory",
        func=lambda _: format_history(),
        description="Displays the full conversation history so far. Use when you need context recall or to summarize past turns."
    )
    tools.append(memory_tool)

    # Create enhanced react prompt template
    template = """You are a helpful AI assistant with access to multiple tools. Answer questions as accurately as possible using the available tools.
...
Thought:{agent_scratchpad}"""
    ...
    # Wrap with message history for stateful conversations
