...
from langchain_core.runnables.history import RunnableWithMessageHistory  # ensure imported
...
# Postgres connection settings for async chat history (set via environment)
PG_HOST = os.getenv("PG_HOST", "localhost")
PG_PORT = os.getenv("PG_PORT", "5432")
PG_USER = os.getenv("PG_USER", "postgres")
PG_PASSWORD = os.getenv("PG_PASSWORD", "postgres")
PG_DB = os.getenv("PG_DB", "chat_history")
PG_DSN = f"postgresql://{PG_USER}:{PG_PASSWORD}@{PG_HOST}:{PG_PORT}/{PG_DB}"  # asyncpg DSN

class AsyncPGChatMessageHistory(BaseChatMessageHistory):
    """Chat message history stored in PostgreSQL using asyncpg.
    Exposes synchronous methods expected by LangChain by internally running
    ...
    Table schema (auto-created):
        ...
        CREATE INDEX IF NOT EXISTS idx_message_store_session_created_at
            ON message_store(session_id, created_at);
    """
    _pool = None  # class-level connection pool

    def __init__(self, session_id: str, dsn: str = PG_DSN, table_name: str = "message_store"):
        self.session_id = session_id
        self._dsn = dsn
        self.table_name = table_name
        self._ensure_loop()
        self._run(self._ensure_pool_and_schema())
    ...
    # ------------- DB setup -------------
    async def _ensure_pool_and_schema(self):
        if self.__class__._pool is None:
            self.__class__._pool = await asyncpg.create_pool(self._dsn, min_size=1, max_size=5)
        async with self.__class__._pool.acquire() as conn:
            ...
            """)
            await conn.execute(
                f"CREATE INDEX IF NOT EXISTS idx_{self.table_name}_session_created_at ON {self.table_name}(session_id, created_at)"
            )
    ...
    async def _fetch_rows(self):
        async with self.__class__._pool.acquire() as conn:
            return await conn.fetch(
                f"SELECT role, content FROM {self.table_name} WHERE session_id=$1 ORDER BY created_at ASC",
                self.session_id,
            )

    @property
    def messages(self) -> list[BaseMessage]:  # type: ignore[override]
        rows = self._run(self._fetch_rows())
        out: list[BaseMessage] = []
        for r in rows:
            if r[0] == "human":
                out.append(HumanMessage(content=r[1]))
            else:
                out.append(AIMessage(content=r[1]))
        return out

    # ------------- insert helpers -------------
    async def _insert(self, role: str, content: str):
        async with self.__class__._pool.acquire() as conn:
            await conn.execute(
                f"INSERT INTO {self.table_name} (session_id, role, content) VALUES ($1, $2, $3)",
                self.session_id,
                role,
                content,
            )
...
def main():
    ...
    # ---- Conversation memory (PostgreSQL asyncpg) ----
    def get_session_history(session_id: str) -> AsyncPGChatMessageHistory:
        return AsyncPGChatMessageHistory(session_id=session_id, dsn=PG_DSN)
    # -----------------------------------------------
    ...
    memory_tool = Tool(
        ...
        description="Displays the full conversation history so far (persisted in PostgreSQL). Use when the user asks to see conversation history, prior messages, or memory."
    )
